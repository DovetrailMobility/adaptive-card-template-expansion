import assert from "assert";

import formPayload from "./contact-us-form-request.json" assert { type: "json" };
import expectedParsedFormPayload from "./contact-us-form-response.json" assert { type: "json" };

const functionUrl = process.env.CONTACT_US_FUNCTION_INVOKE_URL;

async function getParsedFormPayload(formPayload) {
  const options = {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify(formPayload),
  };

  const response = await fetch(functionUrl, options)
    .then((res) => res.json())
    .then((json) => {
      return json;
    })
    .catch((err) => {
      console.error(err);
      return err;
    });

  return response;
}

describe("ContactUsFormParserService", () => {
  it("should return the expected form payload", async () => {
    const actualParsedFormPayload = await getParsedFormPayload(formPayload);
    assert.deepStrictEqual(actualParsedFormPayload, expectedParsedFormPayload);
  });

  it("should return an error when form payload is undefined", async () => {
    const expectedParsedFormPayload = {
      error:
        "An error occurred processing the request: Request body is missing or empty.",
    };
    const actualParsedFormPayload = await getParsedFormPayload(undefined);
    assert.deepStrictEqual(actualParsedFormPayload, expectedParsedFormPayload);
  });
});
