...
app.http('AdaptiveCardTemplateExpansionService', {
    ...
    handler: async (request, context) => {
        ...
        try {
            ...
            if (templatePayload.$schema !== 'http://adaptivecards.io/schemas/adaptive-card.json' || typeof templatePayload.$schema === undefined) {
                throw new Error('Template $schema is missing or invalid.');
            };

            if (templatePayload.type !== 'AdaptiveCard' || typeof templatePayload.type === undefined) {
            ...
            if (!cardPayload || !cardPayload.type || cardPayload.type !== 'AdaptiveCard') {
            ...
            return {
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ "error": `An error occurred processing the request: ${error.message}`}),
                status: 500
            };
        }
    },
